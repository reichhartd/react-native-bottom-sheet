...
>(function BottomSheetModal(props, ref) {
  ...
  const handleExpand = useCallback<BottomSheetModal['expand']>((...args) => {
    if (minimized.current) {
      return;
    }
    bottomSheetRef.current?.expand(...args);
  }, []);
  const handleCollapse = useCallback<BottomSheetModal['collapse']>(
    (...args) => {
      if (minimized.current) {
        return;
      }
  ...
  const handleClose = useCallback<BottomSheetModal['close']>((...args) => {
    if (minimized.current) {
      return;
    }
    bottomSheetRef.current?.close(...args);
  }, []);
  const handleForceClose = useCallback<BottomSheetModal['forceClose']>(
    (...args) => {
      if (minimized.current) {
        return;
      }
